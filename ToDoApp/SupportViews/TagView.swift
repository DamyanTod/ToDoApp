...
@objc protocol TagViewDelegate : class
{
    ...
    @objc optional func getTagValuesAfterEdit( editedItems : [String] )
...
class TagView: UIView, UITextFieldDelegate
{
	...
	var _font						: UIFont!
	var _tagBackgroundColor			: UIColor			  = UIColor.defaultLightGreenColor()
	var _tagSelectedColor			: UIColor			  = UIColor.gray
	...
	var _enableEdit					: Bool				  = false
	var _maxTagCount				: Int				  = 1000
	var _tagCount					: Int				  = 0
	var _targetTextField			: UITextField!
    var _isSelected                 : Bool                = false {
        didSet {
            _tagBackgroundColor = _isSelected ? UIColor.defaultLightGreenColor() : UIColor.gray
        }
    }
    fileprivate var itemsArray      : [String]            = []
	...
	convenience init( position: CGPoint, size: CGSize, options: [TagViewOptions]? = nil )
	{
		...
		_addButton.setBackgroundImage( UIImage.fromColor( color: UIColor.white ), for: .highlighted )
	...
	// layout functions
	// --------------------------------------------------------------------------------------------
    func addTag( tagString: String, tagId : Int? , isSelected : Bool , userData: Any? = nil )
	{
        ...
        tagField.backgroundColor	= isSelected ? _tagBackgroundColor : UIColor.gray
        ...
        tagField.isSelected         = isSelected
	...
	func fitTag( tagField: UITextField )
	{
		if 0 < tagField.text!.characters.count
		{
			...
			{
				tagField.text = _tagPrefix + tagField.text!.lowercased()
			}
		}
		tagField.sizeToFit()
...
//		_tagCharCountLabel.text			  = _targetTextField.text!.characters.count.description + " / " + _tagMaxCharactors.description
	...
	func onTapAddButton( sender: UIButton )
	{
		...
		{
			addTag( tagString: _tagPrefix, tagId: nil , isSelected: false )
			_tagCount += 1
            _addButton.isHidden = true
	...
	func textFieldShouldReturn( _ textField: UITextField ) -> Bool
	{
        ...
        delegate?.getTagValuesAfterEdit?(editedItems: takeTextsFromAllTags())
		return true
	}
    
    private func takeTextsFromAllTags () -> [String] {
        var result = Set<String>()
        for (_ , value) in _tagFields {
	...
	func onTouchEnd( timer: Timer )
	{
        ...
        _tagFields[ _touchTagIndex ]!.backgroundColor = _tagFields[ _touchTagIndex ]!.isSelected ? UIColor.gray : _tagBackgroundColor
        
        _tagFields[ _touchTagIndex ]!.isSelected = _tagFields[ _touchTagIndex ]!.isSelected ? false : true
        
        collectPressedItems(key : ( _tagFields[ _touchTagIndex ]?.text  )! , isSelected: _tagFields[ _touchTagIndex ]?.isSelected)
        
        delegate?.itemPressed(pressedItems: itemsArray)
        
		//delegate?.onTapTag!( name: ( _tagFields[ _touchTagIndex ]?.text )!, userData: _userDatum[ _touchTagIndex ] )
	}
    
    fileprivate func collectPressedItems (key : String?, isSelected : Bool?) {
        if (isSelected ?? false) {
            itemsArray.append(key ?? "")
        } else {
            var index:Int = 0
